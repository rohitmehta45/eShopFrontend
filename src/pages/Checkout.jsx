...
function CheckoutForm() {
  ...
  const submitEsewaForm = ({
    ...
    if (!gatewayUrl) {
      throw new Error(
        'eSewa gateway URL is missing.'
      );
    }

    if (!fields) {
      throw new Error(
        'eSewa payment fields are missing.'
      );
    }
  ...
  const methods = [
    [
      'esewa',
      'eSewa Sandbox',
      'Complete a secure sandbox payment on eSewa.',
    ],
  ];
    ...
    >
        ...
        <div className="mt-4 space-y-3">
          {methods.map(
            ([
              ...
              <label
                key={value}
                className={`flex cursor-pointer items-center gap-3 rounded-[14px] border p-4 ${
                  form.paymentMethod ===
                  value
          ...
          <p className="rounded-[12px] border border-[var(--color-border)] bg-ivory p-3 text-xs text-[var(--color-text-secondary)]">
            Online options use sandbox
            gateways only. Payment is marked
            paid only after backend
            verification.
          </p>
        </div>
      </section>
      ...
      <section>
        ...
        <div className="mt-4 rounded-[14px] bg-ivory p-5 text-sm text-mocha">
          ...
          <div className="mt-3 flex justify-between border-t border-[var(--color-border)] pt-3 text-lg font-semibold text-espresso">
            <span>Total</span>
        ...
        <button
          disabled={loading}
          className="btn-primary mt-4 w-full py-3 disabled:cursor-not-allowed disabled:opacity-60"
          type="submit"
        >
          {loading
            ? 'Processing payment...'
      ...
      <div className="grid gap-8 lg:grid-cols-[1.5fr_0.8fr]">
        ...
        <aside className="card h-fit p-6">
          ...
          <div className="mt-5 space-y-3 text-sm text-mocha">
            ...
            <div className="flex justify-between border-t border-[var(--color-border)] pt-3 text-lg font-semibold text-espresso">
              <span>Total</span>
